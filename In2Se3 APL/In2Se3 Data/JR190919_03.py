...
def plot_delta_Vgmax():
    ...
    (ax, ax2, ax3) = jpp.pretty_plot(fig, labels=["$\it{T}$ (K)", '$\it{ΔV_{G}^{max}}$ (V)', '$\it{@I_{D}}$ (nA)'],
    ...
    jpp.save_generic_svg(fig, fileroot, "_Vg_max")
...
def plot_IDSvsVg_effect_heating():
    ...
    inse.plot_IDvsVg_generic(fileroot, files, '_JR190919_03_effect_heating', colors, log=True, size=2, \
...
def plot_IDSvsVDS_effect_heating(figsize=2):
    ...
    inse.plot_IDvsVDS_generic(fileroot, files, '_JR190919_03_effect_heating', colors,  \
...
def remove_leak_by_fit(file, start=-75., end=-75., mid=75., flat1=-40., flat2=40.):
    ...
    occ0 = jpp.first_occurance_1D(file['Gate_Voltage_V'], start, tol=0.2, starting_index=0)
    occ1 = jpp.first_occurance_1D(file['Gate_Voltage_V'], end, tol=0.2, starting_index=occ0+1)
    ...
    occ_flat0 = jpp.first_occurance_1D(V_G, flat1, tol=0.2, starting_index=0)
    occ_flat1 = jpp.first_occurance_1D(V_G, flat2, tol=0.2, starting_index=0)
    ...
    occ_mid = jpp.first_occurance_1D(V_G, mid, tol=0.2, starting_index=0)
    ...
    occ_flat3 = jpp.first_occurance_1D(V_G, flat2, tol=0.2, starting_index=occ_mid) + occ_mid
    occ_flat4 = jpp.first_occurance_1D(V_G, flat1, tol=0.2, starting_index=occ_mid) + occ_mid
...
def plot_loopI():
    ...
    index0V = [jpp.first_occurance_1D(file['Gate_Voltage_V'], 0) for file in files]
    index25V = [jpp.first_occurance_1D(file['Gate_Voltage_V'], 25) for file in files]
    index50V = [jpp.first_occurance_1D(file['Gate_Voltage_V'], 50) for file in files]
    index75V = [jpp.first_occurance_1D(file['Gate_Voltage_V'], 75) for file in files]
    ...
    #plt.tight_layout()
    jpp.save_generic_svg(fig, fileroot, "_loop_I")       
...
def plot_300K_IDvsVDS(figsize=2, log=False):
    ...
    inse.plot_IDvsVDS_generic(fileroot, files, savename + '_positive_increasing', colors,\
                              figsize=figsize, log=log, xadj=0)
    ...
    inse.plot_IDvsVDS_generic(fileroot, files, savename + '_negative_increasing', colors,\
                              invertaxes=True, figsize=figsize, log=log, xadj=0)
...
def plot_IDSvsT_cooling_vs_warming():
    ...
    fig = plt.figure(figsize=(1.5, 1.5), dpi=300)
    ax = jpp.pretty_plot_single(fig, labels=["$\it{T}$ (K)", '$\it{I_{D}}$ (A)'],
    ...
    jpp.save_generic_svg(fig, fileroot, "_JR190919_03_IDS75V(T)_cooling_vs_warming")
...
def plot_RT4p(size=2):
    ...
    ax = jpp.pretty_plot_single(fig, labels=["$\it{T}$ (K)", '$\it{R}$ (Ω)'],
    ...
    jpp.save_generic_svg(fig, fileroot, "_JR190919_03_R4pt(T)_coolwarm")
...
def plot_R_DSvT_4p(size=2):
    ...
    fig = plt.figure(figsize=(size, size), dpi=300)
    ax = jpp.pretty_plot_single(fig, labels=["$\it{T}$ (K)", '$\it{R_{DS}}$ (Ω)'],
                             yscale='log', fontsize=10, labelsize=10)
    ...
    jpp.save_generic_svg(fig, fileroot, "_JR190919_03_R_DSvT_4p_coolwarm")
...
def plot_IDSvsVg_Vs(): # unused, not much useful to say
    ...
    files = [jpp.process_file(os.path.join(fileroot, x)) for x in filenames]
    inse.rate_300K_filenames(fileroot, files, '_JR190919_03_Vs', colors)

def calc_field_effect_mobility(fileroot, filenames, volt_range=(0,10)):
    files = [jpp.process_file(os.path.join(fileroot, x)) for x in filenames]
    files = inse.slice_data_each(files, "Voltage_1_V", volt_range[0], volt_range[1],\
...
def main(): #Sample C
    ...
    if False or show_all:
        inse.plot_IDvsVg_each(fileroot, RTloop_filenames, '_JR190919_03', log=True, size=figsize, majorx=40,
                          ylim=(None,None), fontsize=10, labelsize=10)
        inse.plot_IDvsVg_each(fileroot, [RTloop_filenames[3]], '_JR190919_03', log=True, size=figsize, majorx=40,
                          ylim=(10**-12, 1*10**-6), fontsize=10, labelsize=10) # adjust for text
    
     # -- Cross section of loop data --
    if False or show_all:
        inse.plot_loopR_cross_section(fileroot, RTloop_filenames, "_JR190919_03_RDS", increments=[0,25,50,75],\
                                      figsize=figsize, log=True, ylim=(None, None))
        inse.plot_loopR_cross_section(fileroot, RTloop_filenames, "_JR190919_03_RDSv2", increments=[50,75],\
    ...
    # -- carrier mobility μ
    if False or show_all:
        inse.plot_mobility_μ_cross_section(fileroot, RTloop_filenames, "_JR190919_03", JR190919_03_length, JR190919_03_width, figsize=1.5, ylim=(None, None),\
