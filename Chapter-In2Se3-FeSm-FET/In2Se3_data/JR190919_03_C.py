...
JR190919_03 = mp.flake_device()
fileroot = "JR190919_03"
JR190919_03.fileroot = fileroot
JR190919_03.name = "JR190919_03"
JR190919_03.width = 10*10**-6 # meters
JR190919_03.length = 20*10**-6 # meters
JR190919_03.volt_spacing = 8*10**-6 # meters (center to center)
JR190919_03.thickness = 110*10**-9 # meters
...
def main(): #Sample C
    ...
    # Plot ID vs VG loops
    if False or show_all:
        mp.plot_IDvsVg_each(JR190919_03, RTloop_filenames, '_JR190919_03', log=False, size=figsize, majorx=40,
                          ylim=(None,None), fontsize=10, labelsize=10)
        mp.plot_IDvsVg_each(JR190919_03, RTloop_filenames, '_JR190919_03', log=True, size=figsize, majorx=40,
                          ylim=(None,None), fontsize=10, labelsize=10)
        mp.plot_IDvsVg_each(JR190919_03, [RTloop_filenames[3]], '_JR190919_03', log=True, size=figsize, majorx=40,
                          ylim=(10**-12, 1*10**-6), fontsize=10, labelsize=10) # adjust for text
    
     # -- Cross section of loop data --
    if False or show_all:
        mp.plot_loopR_cross_section(JR190919_03, RTloop_filenames, "_JR190919_03_RDS", increments=[0,25,50,75],\
                                      figsize=figsize, log=True, xlim=(0,322), ylim=(None, None))
        mp.plot_loopR_cross_section(JR190919_03, RTloop_filenames, "_JR190919_03_RDSv2", increments=[50,75],\
    ...
    # -- carrier mobility μ
    if False or show_all:
        mp.plot_mobility_μ_cross_section(JR190919_03, RTloop_filenames, "_JR190919_03", 
    ...
    if False or show_all:
        mp.plot_maxSS_vs_T(JR190919_03, RTloop_filenames, '_minSSvsT', Npoints=5, Icutoff=2*10**-12)
    
    # delta voltage threshold
    if False or show_all:
        mp.plot_ΔVTvT(JR190919_03, RTloop_filenames, '_ΔVTvsT', Npoints=5, Icutoff=1*10**-11)
    
    # Schotty and Schotty-Simmons for fixed V. ln(J*T^pow) vs 1/T
    if False or show_all:
        mp.plot_Schottky_Simmons_cross_section(JR190919_03, RTloop_filenames, "_JR190919_03", 
                figsize=2, xlim=(0,None),
                ylim=(None, None), increments=[0, 25, 50, 75], colororder=[0,3,2,1])
        mp.plot_Schottky_cross_section(JR190919_03, RTloop_filenames, "_JR190919_03", 
                figsize=2, xlim=(0,None),
                ylim=(None, None), increments=[0, 25, 50, 75], colororder=[0,3,2,1])
