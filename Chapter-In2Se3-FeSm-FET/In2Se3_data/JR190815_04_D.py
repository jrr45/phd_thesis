...
JR190815_04 = mp.flake_device()
fileroot = r"JR190815_04"
JR190815_04.name = "JR190815_04"
JR190815_04.fileroot = fileroot
JR190815_04.thickness = 110*10**-9 # meters (thickness)
JR190815_04.length = 12*10**-6 # meters
JR190815_04.width = 8*10**-6 # meters
JR190815_04.volt_spacing = 5*10**-6 # meters (center to center)             
...
def process_hall_data():
    ...
    (cs_Currents_left, cs_Voltages_left, _, cs_Temperatures) = \
        mp.get_cross_section(JR190815_04, RTloop_2_4pt_filenames, [75.], 1)
    (cs_Currents_right, cs_Voltages_right, _, cs_Temperatures) = \
        mp.get_cross_section(JR190815_04, RTloop_2_4pt_filenames, [75.], 2)
    ...
    for (file, off) in zip(files, offset):
        ...
        σs = JR190815_04.length / (R_2pt * JR190815_04.width)
        ...
        #σs = l/(Rxx*w)
        σs = JR190815_04.volt_spacing / (Rxx_4pt * JR190815_04.width)
...
def plot_n_hall_3D():
    ...
    n3D = np.divide(n2D,JR190815_04.thickness)/(100.0**3.0)
...
def get_300K_IDvsVDS_files():
    start = 10
    basename = 'JR190815_04_'
    
    filenames = [(basename + str(i).zfill(3) + '_IvsV_increase.txt') for i in range(start, start+7)]
    files_pos_inc = [mp.process_file(os.path.join(fileroot, x)) for x in filenames]
    
    filenames = [(basename + str(i).zfill(3) + '_IvsV_increase.txt') for i in range(start+7, start+14)]
    files_pos_dec = [mp.process_file(os.path.join(fileroot, x)) for x in filenames]
    
    filenames = [(basename + str(i).zfill(3) + '_IvsV_decrease.txt') for i in range(start+14, start+21)]
    files_neg_inc = [mp.process_file(os.path.join(fileroot, x)) for x in filenames]
    
    filenames = [(basename + str(i).zfill(3) + '_IvsV_decrease.txt') for i in range(start+21, start+28)]
    files_neg_dec = [mp.process_file(os.path.join(fileroot, x)) for x in filenames]
...
def main(): #sample D
    ...
    if False or show_all:
        mp.plot_IDvsVg_each(JR190815_04, RTloop_filenames, '_JR190815_04', log=True, size=2, majorx=40,
                          ylim=(None,None), fontsize=10, labelsize=10)
    
    # -- Cross section of loop data --
    if False or show_all:
        mp.plot_loopR_cross_section(JR190815_04, RTloop_filenames, "_JR190815_04_RDS", increments=[0,25,50,75],\
    ...
    # -- carrier mobility μ
    if False or show_all:
        mp.plot_mobility_μ_cross_section(JR190815_04, RTloop_filenames, "_JR190815_04",
                                         figsize=1.5, ylim=(None, None),\
                                         log=False, increments=[25, 50, 75], colororder=[3,2,1])
        
    # min subthreshold slope
    if False or show_all:
        mp.plot_maxSS_vs_T(JR190815_04, RTloop_filenames, '_minSSvsT', Npoints=5, Icutoff=10*10**-11)
