...
def plot_rho_ln(figsize=(2, 2), fit=True):
    colors = mp.colors_set1
    filenames = ['Bulk_rect_010_RvsT_hall.txt',
                 'Bulk_rect_002_RvsT_c-plane.txt']
    files = [mp.process_file(os.path.join(fileroot, x)) for x in filenames]
    ...
    if fit:
        logrhoxxc = np.log(rhoxxc)
        coeffs, R2, xfit, yfit = \
            mp.fit_to_limit(np.flip(temperaturesc), np.flip(logrhoxxc), R2=.995,  points=5)
        
        yfit = np.exp(yfit)
        ax.plot(xfit, yfit*.001, '.-', ms=0, linewidth=1.5, color='black')
        print("T_0c = %s K" % str(np.power(coeffs[0], 4)))
        
        logrhoxxab = np.log(rhoxxab)
        coeffs, R2, xfit, yfit = \
            mp.fit_to_limit(np.flip(temperaturesab), np.flip(logrhoxxab), R2=.995,  points=5)
        
        yfit = np.exp(yfit)
        ax.plot(xfit, yfit*.001, '.-', ms=0, linewidth=1.5, color='black')
        print("T_0ab = %s K" % str(np.power(coeffs[0], 4)))
